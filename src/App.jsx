...
import { Download, Copy, Check, Film, Search, Tv, AlertCircle } from 'lucide-react';
...
export default function App() {
  ...
  const fetchTrendingMovies = async () => {
    ...
    try {
      ...
      setError('Erro ao carregar filmes. Verifique sua VITE_TMDB_API_KEY.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=${encodeURIComponent(search)}&language=pt-BR`
      );
      const data = await res.json();
      setMovies(data.results || []);
    } catch (err) {
  ...
  // Busca a URL real via API serverless (resolve CORS e Mixed Content)
  ...
  // Download direto: usa <a download> para forçar o browser a baixar
  const handleDownload = async (movie) => {
    ...
    try {
      ...
      link.setAttribute(
        'download',
        `${(movie.title || 'filme').replace(/[^a-zA-Z0-9]/g, '_')}.mp4`
      );
      link.target = '_blank'; // fallback: abre em nova aba se o download for bloqueado
  ...
  // Copia o link real da CDN para a área de transferência
  const handleCopyLink = async (movie) => {
    ...
    try {
      ...
      setError(`Erro ao copiar link: ${err.message}`);
    }
  };
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {movies.map((movie) => {
              ...
              const isDownloading = downloadingId === movie.id;
              const isCopied = copiedId === movie.id;

              return (
                <div
                  key={movie.id}
                  className="bg-[#0e1322] rounded-xl overflow-hidden border border-slate-900 hover:border-slate-800/80 transition-all flex flex-col group shadow-lg"
                >
                  ...
                  <div className="p-3 flex flex-col flex-grow justify-between bg-[#0e1322]">
                    ...
                    <div className="space-y-1.5 mt-auto">
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-[11px] font-bold bg-cyan-500 hover:bg-cyan-400 text-slate-950 transition-colors shadow-md shadow-cyan-500/5 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        <Download className="w-3.5 h-3.5" />
                        {isDownloading ? 'Obtendo link...' : 'Download Direto'}
                      </button>

                      <button
                        onClick={() => handleCopyLink(movie)}
                        disabled={isDownloading}
                        className={`w-full flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-[10px] font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                          isCopied
